...
(function(){
  ...
  function icon(name){
    var paths={
      ...
      equipe:'<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="m17 11 2 2 4-4"/>',
  ...
  var NAV=[
    ...
    ['Áreas',[
      ...
      ['internet.html','Internet','internet','areas'],['cenografia.html','Cenografia','ceno','areas'],['extras.html','Extras','extras','areas'],['servicos.html','Serviços','servicos','areas'],['logistica.html','Logística','logistica','logistica'],['equipe.html','Equipe','equipe','equipe']
    ]],
    ['Documentos',[
      ['descritivo-geral.html','Descritivo Geral','descricao','areas'],['ordem-servico.html','Ordem de Serviço','os','os'],['os-geradas.html','OS Geradas','os','all']
    ]],
    ['Sistema',[
      ['usuarios-permissoes.html','Usuários','usuarios','users'],['log-auditoria.html','Log de auditoria','auditoria','audit']
    ]]
  ];
  ...
  function canShow(scope){
    ...
    if(scope==='audit') return EF.canSeeUsuarios ? EF.canSeeUsuarios() : false;
    if(scope==='salesorder') return !!(EF.profile && ['gerencia','producao'].indexOf(EF.profile.role)!==-1);
    if(scope==='os') return EF.canSeeOS ? EF.canSeeOS() : false;
  ...
  function setupShell(){
    ...
    // Algumas páginas (ex.: Ordem de Serviço) possuem containers auxiliares
    // antes do aplicativo, como #print-os. Nunca assuma que o primeiro DIV
    // do body é o shell principal. Localize o container que realmente possui
    // sidebar + conteúdo principal.
    var shell=document.querySelector('.ef-app-shell');
    if(!shell){
      var candidates=Array.prototype.slice.call(document.body.children).filter(function(el){
        if(!el || el.tagName!=='DIV') return false;
        if(el.id==='print-os' || el.classList.contains('ef-backdrop')) return false;
        return el.children && el.children.length>=2;
      });
      shell=candidates.find(function(el){
        var first=el.children[0], second=el.children[1];
        return first && second && first.tagName==='DIV' && second.tagName==='DIV';
      }) || candidates[0];
    }
    if(!shell) return;
  ...
  EF_UI.scrollTo=function(target){
    var el=typeof target==='string'?document.querySelector(target):target;
    if(!el) return;
    setTimeout(function(){try{el.scrollIntoView({behavior:'smooth',block:'center'});}catch(e){}},80);
  };
